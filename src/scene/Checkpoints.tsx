...
import { useStore, type Checkpoint } from "../state/store";

const ACTIVE = 0xff5d3b;
const IDLE = 0x4a5450;
const START = 0xe0b15e;

function Gate({ cp, active, isStart }: { cp: Checkpoint; active: boolean; isStart: boolean }) {
  const beam = useRef<THREE.Mesh>(null);
  const color = active ? ACTIVE : isStart ? START : IDLE;

  useFrame(({ clock }) => {
    if (!beam.current) return;
    const m = beam.current.material as THREE.MeshBasicMaterial;
    m.opacity = active ? 0.12 + 0.07 * Math.sin(clock.elapsedTime * 4) : 0.04;
  });
  ...
  return (
    <group>
      {checkpoints.map((cp, i) => (
        <Gate key={cp.id} cp={cp} active={i === nextCp} isStart={i === 0} />
      ))}
    </group>
  );
}
